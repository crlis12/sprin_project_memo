...
public class PostBO {
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		// 2) 이전을 눌렀을 때 : prevId: 7 7보다 큰 3개(ASC) => 코드에서 list reverse
		...
		if (prevId != null) {
			...
			// 7보다 큰 3개(ASC) 8 9 10 => reverse 시킨다. 10 9 8 
	...
	// update
	public int updatePost(String userLoginId, int userId, int postId, String subject, String content,
			MultipartFile file) {

		Post post = this.getPostById(postId);
		if (post == null) {
			// System.out.println(""); // 웹에서는 사용하면 안되다 쓰레드가 멈추기 때문에 사용하지 않는다 동시에 3명의 사용자가
			// 접속하면 한명의 사용자만 보여지고 나머지는 동작이 맘추기 떄문
